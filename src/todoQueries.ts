...
export async function addTodo(Description: string, date: string, picture: string, userId: number): Promise<number> {
    const [{ insertId }] = await sql.execute('INSERT INTO todos (Description, date, picture, UserId) VALUES (?, ?, ?, ?)', [Description, date, picture, userId]);
    return insertId;
}
